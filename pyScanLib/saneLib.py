...
class saneLib(object):
    ...
    def setScanArea(self, left=0.0, top=0.0, width=8.267, height=11.693):
        ...
        self.scanner.tl_x = float(self.inchTomm(left))
        # top left y axis           top
        self.scanner.tl_y = float(self.inchTomm(top))
        # bottom left x axis      width
        self.scanner.br_x = float(self.inchTomm(width))
        # bottom left y axis     height
        self.scanner.br_y = float(self.inchTomm(height))
    ...
    def getScannerSize(self):
        ...
        return (self.mmToInch(self.scanner.tl_x), self.mmToInch(self.scanner.tl_y), self.mmToInch(self.scanner.br_x), self.mmToInch(self.scanner.br_y))
